...
Here we try to divide the study period into phases which have resembling SB usage.
\begin{Definition}[k-division]
Let $X=\{x_1,x_2,\ldots,x_N\}$ be a time serie of length $N$.
Given positive integer $k<N$ and an index set $\{n_0,\ldots,n_k\}$ with $n_0=0$ and $n_k=N$, a k-division of $X$ is the set of time series $X_i=\{x_{n_i+1},\ldots,x_{n_{i+1}}\}$ where $1\leq i\leq k$ and $\forall n_i$, $n_i<n_{i+1}$.
\end{Definition}

To evaluate the quality of k-division, we use the sum of standard deviation $\sigma=\sum_{i=1}^{k}{\sigma_i}$ where $\sigma_i$ is the standard deviation of division $X_i$. %$\sqrt{\frac{1}{n}\sum_{i=1}^{n}{(x-\bar{x})^2}}$ 
The optimal solution can be found via enumeration if $k$ and $N$ are small.
...
According to ratio $a/n$, the SB usage of infected RAs and their surrounding RAs have recovered roughly to the same level like other RAs from the pandemic.
