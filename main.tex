...
\begin{Definition}[k-division]
Let $X=\{x_1,x_2,\ldots,x_N\}$ be a time serie of length $N$.
Given positive integer $k<N$ and index set $\mathbf{T}=\{n_0,\ldots,n_k\}$ with $n_0=0$, $n_k=N$ and $\forall i$, $n_i<n_{i+1}$, a k-division of $X$ is the set of time series $X_i=\{x_{n_i+1},\ldots,x_{n_{i+1}}\}$ where $0\leq i\leq k-1$.
\end{Definition}

To evaluate the quality of k-division, we use $\sigma=\sum_{i=1}^{k}{\sigma_i}$ as loss function where $\sigma_i$ is the standard deviation of division $X_i$. %$\sqrt{\frac{1}{n}\sum_{i=1}^{n}{(x-\bar{x})^2}}$ 
The optimal solution is formulated as $\arg\min_{\mathbf{T}}\sigma(\mathbf{T})$ and can be found via enumeration if $k$ and $N$ are small.

\subsection{Basic statistics}
Spatial analyst and statistics concerning the evolution of SB usage over time in different zones of Beijing, are implemented with the ArcGIS 10.7 and Python programming language. 
...
We want to classify the period into three phases: before pandemics, during pandemics, pandemics mitigated.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|l|l|l|l|l|l|l|}
        ...
        \cline{3-7}
        & & 01 Jan-21 Jan ($b$) & 22 Jan-16 Feb ($m$) & 17 Feb-01 Mar ($a$) & $m/b$ & $a/b$\\ %$a/m$\\
        \hline
...
Ratio $a/b$ shows the bike usage of all categories have recovered to some extent but far from the situation of usual time after the partial restart of activities.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|l|l|l|l|l|l|l|}
        ...
        \cline{3-7}
<<<<<<< HEAD
        & & 01 Jan-21 Jan ($a$) & 05 Feb-16 Feb ($b$) & 17 Feb-04 Mar ($c$) & $b/a$ & $c/a$\\
=======
        & & 01 Jan-21 Jan ($b$) & 05 Feb-12 Feb ($m$) & 13 Feb-04 Mar ($a$) & $m/b$ & $a/b$\\
>>>>>>> bd5fe880f56a515db5275fff4f0349e79742307a
        \hline
...
According to ratio $a/m$, the SB usage of infected RAs and their surrounding RAs have recovered roughly to the same level like other RAs from the pandemic.
...
\textbf{Phase b}: dates between 22 Jan and 09 Feb, include Chinese New Year holiday 2020. During this period, traffic suspension and home quarantine were implemented for pandemics prevention purposes.
...
\ref{fig:compare_2019_and_2020} delineates the results corresponding to 2020, 2019, and annual difference between them.
Panel(a)-(c) were consistent with the daily patterns during different pandemic stages presented in the previous section.
Panel(d) summarized the averaged share-bike intensity during the same time-interval as phase a in 2019, showing similar spatial patterns as 2020. 
Comparable numbers of positive (Red colored) and negative(blue colored) area could be observed from annual difference presented in panel (g). These discrepancies have relatively low values which could be attributed to the fluctuation of regional bike usages requirements.  
...
Motivated by the current COVID-19 outbreak and rapid development in BSS, this paper represents the period-wise spatio-temporal patterns and co-location patternsof the SBs during the pandemic in Beijing, China.
...
The influences of pandemic on individuals' mobility and city's rehabilitation process have been investigated.
Apart from the holiday shutdowns, residents' activities were affected by the COVID-19, which is xxx that of the same period in 2019. 
Productive and social activities partially restarted when the epidemic was mitigated.
1/3 of the vitality of the same period in previous years had been restored, and the recovery process will accelerate according to the trends in spatiotemporal patterns.
The cumulative confirmed cases stayed stable substantially after restarting of productive activities, indicating control measures were necessary and effective in containment of the spread of COVID-19 when opening-up.   
%TODO co-location的结果显示几种POI在疫情阶段的访问情况。
\textcolor{red}{These results could facilitate research and inform policy making in the context of the current COVID-19 outbreak, and help to prevent the emergence of pandemics.} 
However, the government implemented multiple interventions at the same time or in a short timeframe to control the outbreak, therefore individual strategies could not be evaluated.
