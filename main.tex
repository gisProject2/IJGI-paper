...
Identification of the key time nodes in different pandemic period was conducted by segmenting the whole time series while keeping the maximal similarity inside the segments.
In different pandemic periods, we implemented co-location analysis between the usage patterns and different types of POIs, which reveals the public reactions and assess the impact of the COVID-19 pandemic.
...
Therefore, the impact of the COVID-19 pandemic and the influence of suppression strategy could be analyzed on a citywide scale, providing references in more details for policy-making and epidemiology research. 
...
Discussions are presented in Section \ref{sec:discussions} on comparison with the previous year, and relation with confirmed cases during different pandemic periods.
...
\subsection{Methods}
To deal with large-scale spatial queries huge amount of BSS spatiotemporal data (66.8GB), the computation in this paper was done on a computing cluster consisting of 5 machines with Intel(R) Xeon(R), CPU E5-2640 v2 @2.00GHz, 8 cores, 61.7GB memory, 20480KB cache. 
...
In addition, two strategies are included to quantify the residents responses to COVID-19.
Subsection \ref{sec:phase_classification} introduces a phase classification strategy to identify key time nodes based on the changes of share bike usgaes corresponding to COVID-19.
The design of co-location analysis strategy is presented in Subsection \ref{sec:co-location}, which is used to quantify the spatial relationships between POIs and share bike usgaes.  
...
\subsection{Temporal patterns of share bike records}
...
\subsection{Spatiotemporal patterns of share bike records}
...
This part aims at characterizing the behavior patterns of disease-spreading individuals by evaluating share bike usage concerning different pandemic periods.%thus provide references for authorities to gradually lift the most restrictive measures with more precision and confidence.
...
We investigated the impact of the pandemic on the behavior of residents and the rehabilitation process in the city scale.
By removing the factor of the Chinese New Year holiday, results exhibit that social and productive activities of residents were hugely affected by the COVID-19, reflected in the drastic decrease of share bike usage (down to less than $40\%$ of that of the same period in 2019). 
The cumulative confirmed cases stayed stable substantially after restarting of productive activities, implying the ongonging control measures were necessary and effective in the containment of the spread of COVID-19 when opening-up.
...
\acknowledgments{This study was supported by National Key R\&D Program of China (2018YFB2100701), and Research Program of Beijing Advanced Innovation Center for Future Urban Design (UDC2019031321), and the National Natural Science Foundation of China(41601389).}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% optional
